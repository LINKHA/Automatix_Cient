...
namespace amx
{
	...
	void server::remove_service(uint32_t serviceid, uint32_t sender, int32_t sessionid)
	{
		...
		{
			auto content = amx::format("remove_service: invalid service id %u.", serviceid);
	...
	std::string server::info() const
	{
		...
		std::string req;
		req.append("[\n");
		req.append(amx::format(R"({"id":0, "socket":%zu, "timer":%zu, "log":%zu, "service":%u, "error":%zu})",
			socket_num(),
			timer_size,
			logger_.size(),
			service_count(),
		...
		{
			req.append(",\n");
			auto v = amx::format(R"({"id":%u, "cpu":%f, "mqsize":%u, "service":%u, "timer":%zu, "alive":%u})",
				w->id(),
				w->cpu_cost_,
				w->mqsize_.load(),
