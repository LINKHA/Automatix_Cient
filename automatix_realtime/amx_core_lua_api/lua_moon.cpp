...
static amx::buffer_ptr_t moon_to_buffer(lua_State* L, int index)
{
    ...
    case LUA_TSTRING:
    {
        ...
        auto buf = amx::message::create_buffer(len);
        buf->write_back(str, len);
        return buf;
    }
    case LUA_TLIGHTUSERDATA:
    {
        amx::buffer* p = static_cast<amx::buffer*>(lua_touserdata(L, index));
        return amx::buffer_ptr_t(p);
    }
    default:
...
{
    lua_service* S = lua_service::get(L);
    auto level = (amx::LogLevel)luaL_checkinteger(L, 1);
    if (S->logger()->get_level() < level)
        return 0;

    amx::buffer line = S->logger()->make_line(true, level, S->id());
...
static int lmoon_loglevel(lua_State* L)
{
    ...
    if(lua_type(L, 1) == LUA_TSTRING)
        S->logger()->set_level(amx::lua_check<std::string_view>(L, 1));
...
extern "C" {
    ...
    lua_pushinteger(L, amx::time::timezone());
...
static int lasio_write(lua_State* L)
{
    ...
    if (flag!=0 && (flag<=0 || flag >=(int)amx::buffer_flag::buffer_flag_max))
    {
        return luaL_error(L, "asio.write param 'flag' invalid");
    }
    bool ok = sock.write(fd, moon_to_buffer(L, 2), (amx::buffer_flag)flag);
    lua_pushboolean(L, ok ? 1 : 0);
    return 1;
}
...
static int lasio_write_message(lua_State* L)
{
    ...
    auto& sock = S->get_worker()->socket_server();
    uint32_t fd = (uint32_t)luaL_checkinteger(L, 1);
    amx::message* m =(amx::message*)lua_touserdata(L, 2);
    if (nullptr == m)
...
static int lasio_make_endpoint(lua_State* L)
{
    ...
    char arr[amx::socket_server::addr_v6_size];
    size = amx::socket_server::encode_endpoint(arr, addr, port);
...
static int lasio_unpack_udp(lua_State* L)
{
    ...
    if (size == 0 || nullptr == str)
        return 0;
    size_t addr_size = ((str[0] == '4') ? amx::socket_server::addr_v4_size : amx::socket_server::addr_v6_size);
