...
using navmesh_type = amx::navmesh;

static int load_static(lua_State* L)
{
    auto meshfile = amx::lua_check<std::string>(L, 1);
    std::string err;
    if (amx::navmesh::load_static(meshfile, err))
    {
        lua_pushboolean(L, 1);
        return 1;
    }
...
static int load_dynamic(lua_State* L)
{
    ...
    auto meshfile = amx::lua_check<std::string>(L, 2);
    std::string err;
...
static int find_straight_path(lua_State* L)
{
    ...
    if (nullptr == p)
        return luaL_error(L, "Invalid navmesh pointer");
    auto sx = amx::lua_check<float>(L, 2);
    auto sy = amx::lua_check<float>(L, 3);
    auto sz = amx::lua_check<float>(L, 4);
    auto ex = amx::lua_check<float>(L, 5);
    auto ey = amx::lua_check<float>(L, 6);
    auto ez = amx::lua_check<float>(L, 7);
...
static int valid(lua_State* L)
{
    ...
    if (nullptr == p)
        return luaL_error(L, "Invalid navmesh pointer");
    auto x = amx::lua_check<float>(L, 2);
    auto y = amx::lua_check<float>(L, 3);
    auto z = amx::lua_check<float>(L, 4);
...
static int random_position_around_circle(lua_State* L)
{
    ...
    auto y = amx::lua_check<float>(L, 3);
    auto z = amx::lua_check<float>(L, 4);
    auto r = amx::lua_check<float>(L, 5);
...
static int add_capsule_obstacle(lua_State* L)
{
    ...
    auto z = amx::lua_check<float>(L, 4);
    auto r = amx::lua_check<float>(L, 5);
    auto h = amx::lua_check<float>(L, 6);
...
static int remove_obstacle(lua_State* L)
{
    ...
    auto obstacleId = amx::lua_check<dtObstacleRef>(L, 2);
...
static int update(lua_State* L)
{
    ...
    if (nullptr == p)
        return luaL_error(L, "Invalid navmesh pointer");
    auto dt = amx::lua_check<float>(L, 2);
    p->update(dt);
    return 0;
}
