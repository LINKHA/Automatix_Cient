...
static int lrand_range(lua_State* L)
{
    ...
    auto res = amx::rand_range(v_min, v_max);
...
static int lrand_range_some(lua_State* L)
{
    ...
    while (v_count > 0)
    {
        auto index = amx::rand_range((size_t)0, vec->size() - 1);
...
static int lrandf_range(lua_State* L)
{
    ...
    auto res = amx::randf_range(v_min, v_max);
...
static int lrand_weight(lua_State* L)
{
    ...
    amx::lua_array_view<int64_t> values{L, 1};
    amx::lua_array_view<int64_t> weights{L, 2};
    ...
    int64_t cutoff = amx::rand_range(int64_t{0}, sum - 1);
...
static int lrand_weight_some(lua_State* L)
{
    ...
    amx::table_to_vector<int64_t>(L, 1, ctx->v);
    amx::table_to_vector<int64_t>(L, 2, ctx->w);
