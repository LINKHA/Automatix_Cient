...
static int json_options(lua_State* L)
{
    ...
    switch (amx::chash_string(name, len))
    {
...
static int concat(lua_State* L)
{
    if (lua_type(L, 1) == LUA_TSTRING)
    {
        ...
        auto buf = new amx::buffer(size, 16);
    ...
    auto buf = new amx::buffer(cfg->concat_buffer_size, cfg->concat_buffer_head_size);
    try
    {
...
static void write_resp(amx::buffer* buf, const char* cmd, size_t size)
{
    buf->write_back("\r\n$", 3);
...
static void concat_resp_one(amx::buffer* buf, lua_State* L, int i, json_config* cfg)
{
    int t = lua_type(L, i);
    switch (t)
    {
    ...
    try
    {
        ...
        if(lua_type(L, 1) == LUA_TSTRING){
            ...
            if(len>0){
                ...
                hash = static_cast<uint32_t>(amx::hash_range(hash_part.begin(), hash_part.end()));
            }
        }
