...
static void check_field_type(lua_State* L, int index, const std::string& proto_name, const std::string& field_name, const std::string& name, const std::string& type, std::vector<std::string>& trace)
{
	...
	else if (0 == ok)
	{
		...
		if (name.empty())
		{
			throw std::logic_error{amx::format("'%s.%s' %s expected, got %s, value '%s'. trace: %s", proto_name.data(), field_name.data(), type.data(), luaL_typename(L, index), value.data(), trace_to_string(trace).data())};
		}
		else
		{
			throw std::logic_error{amx::format("'%s.%s.%s' %s expected, got %s. trace: %s", proto_name.data(), field_name.data(), name.data(), type.data(), luaL_typename(L, index), trace_to_string(trace).data())};
		}
	}
}
...
static void verify_field(lua_State* L, int index, const std::string& proto_name, const std::unordered_map<std::string, proto_field>& proto, const char* field_name, std::vector<std::string>& trace)
{
	...
	{
		throw std::logic_error{amx::format("Attemp to index undefined field: '%s.%s'. trace: %s", proto_name.data(), field_name, trace_to_string(trace).data())};
	}
	else
	{
		...
		case compose_type::T_ARRAY:
		{
			...
			{
				throw std::logic_error{amx::format("'%s.%s' table expected, got %s. trace: %s", proto_name.data(), field_name, luaL_typename(L, index), trace_to_string(trace).data())};
			}

			auto size = array_size(L, index);
			if (size == std::numeric_limits<size_t>::max())
			{
				throw std::logic_error{amx::format("'%s.%s' not meet lua array requirements. trace: %s", proto_name.data(), field_name, trace_to_string(trace).data())};
			}
...
static void do_verify(lua_State* L, const std::string& proto_name, int index, std::vector<std::string>& trace)
{
	...
	{
		throw std::logic_error{amx::format("'%s' table expected, got %s. trace: %s", proto_name.data(), luaL_typename(L, index), trace_to_string(trace).data())};
	}

	auto iter = schema_define.find(proto_name);
	if (iter == schema_define.end())
	{
		throw std::logic_error{amx::format("Attemp using undefined proto: %s. trace: %s", proto_name.data(), trace_to_string(trace).data())};
	}
...
static int load(lua_State* L)
{
	...
	while (lua_next(L, 1) != 0)
	{
		...
		while (lua_next(L, -2) != 0)
		{
			...
			auto container = amx::lua_opt_field<std::string_view>(L, -1, "container", "");
			if (container == "array")
			{
			...
			field.key_type = amx::lua_opt_field<std::string_view>(L, -1, "key_type", "");
			field.value_type = amx::lua_opt_field<std::string_view>(L, -1, "value_type", "");
			lua_pop(L, 1);
