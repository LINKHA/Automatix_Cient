    ...
    protected:
        ...
        std::error_code handshake(const std::shared_ptr<asio::streambuf>& buf, size_t n)
        {
            ...
            if(!http::request_parser::parse(std::string_view{ reinterpret_cast<const char*>(buf->data().data()), n }
                ...
                , header))
            {
                return make_error_code(amx::error::ws_bad_http_header);
            }

            if (version<"1.0" || version>"1.1")
            {
                return make_error_code(amx::error::ws_bad_http_version);
            }

            if (method != "GET"sv)
                return make_error_code(amx::error::ws_bad_method);

            std::string_view connection;
            if (!amx::try_get_value(header, "connection"sv, connection))
                return make_error_code(amx::error::ws_no_connection);

            std::string_view upgrade;
            if (!amx::try_get_value(header, "upgrade"sv, upgrade))
                return make_error_code(amx::error::ws_no_upgrade);

            if (!iequal_string(connection, UPGRADE))
                return make_error_code(amx::error::ws_no_connection_upgrade);

            if (!iequal_string(upgrade, WEBSOCKET))
                return make_error_code(amx::error::ws_no_upgrade_websocket);

            std::string_view sec_ws_key;
            if (!amx::try_get_value(header, "sec-websocket-key"sv, sec_ws_key))
                return make_error_code(amx::error::ws_no_sec_key);

            if (base64_decode(std::string{ sec_ws_key.data(), sec_ws_key.size() }).size() != 16)
                return make_error_code(amx::error::ws_bad_sec_key);

            std::string_view sec_ws_version;
            if (!amx::try_get_value(header, "sec-websocket-version"sv, sec_ws_version))
                return make_error_code(amx::error::ws_no_sec_version);

            if (sec_ws_version != "13"sv)
            {
                return make_error_code(amx::error::ws_bad_sec_version);
            }

            std::string_view protocol;
            amx::try_get_value(header, "sec-websocket-protocol"sv, protocol);
        ...
        bool handle_frame()
        {
            ...
            if (ec)
            {
                if (ec == amx::error::ws_closed && recv_buf_->size()>1)
                {
        ...
        std::error_code decode_frame()
        {
            ...
            {
                return make_error_code(amx::error::ws_bad_unmasked_frame);
            }
            ...
            case ws::opcode::binary:
                if (fh.rsv1 || fh.rsv2 || fh.rsv3)
                {
                    ...
                    return make_error_code(amx::error::ws_bad_reserved_bits);
                }
                break;
            case ws::opcode::incomplete:
                {
                    //not support continuation frame
                    return make_error_code(amx::error::ws_bad_continuation);
                    break;
                }
            default:
                if (!fh.fin)
                {
                    ...
                    return make_error_code(amx::error::ws_bad_control_fragment);
                }
                if (fh.payload_len > PAYLOAD_MIN_LEN)
                {
                    // invalid length for control message
                    return make_error_code(amx::error::ws_bad_control_size);
                }
                if (fh.rsv1 || fh.rsv2 || fh.rsv3)
                {
                    // reserved bits not cleared
            ...
            case PAYLOAD_MID_LEN:
            {
                ...
                if (reallen < PAYLOAD_MID_LEN)
                {
                    ...
                    return make_error_code(amx::error::ws_bad_size);
                }
                break;
            }
            ...
            {
                return make_error_code(amx::error::ws_closed);
            }
            message msg;
