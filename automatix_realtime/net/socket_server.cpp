    ...
    ctx->acceptor.async_accept(c->socket(), [this, ctx, c, w, sessionid, owner](const asio::error_code& e)
    {
        if (!e)
        {
            if (!ctx->reserve.is_open()) {
                ...
                response(ctx->fd, ctx->owner, amx::format("socket_server::accept %s(%d)", ec.message().data(), ec.value()),
        ...
        else
        {
            ...
            response(ctx->fd, ctx->owner, amx::format("socket_server::accept %s(%d)", e.message().data(), e.value()),
                sessionid, PTYPE_ERROR);
        }
                    ...
                    if (millseconds > 0)
                    {
                        ...
                        timer->async_wait([this, c, owner, sessionid, host, port, timer](const asio::error_code& ec) {
                            if (!ec)
                            {
                                ...
                                {
                                    c->close();
                                    response(0, owner, amx::format("connect %s:%d timeout", host.data(), port), sessionid, PTYPE_ERROR);
                                }
                            }
                            });
                    }
                        ...
                        [this, c, host, port, owner, sessionid, timer](const asio::error_code& ec, const tcp::endpoint&)
                        {
                            ...
                            else
                            {
                                if(cancelled_timer>0)
                                    response(0, owner, amx::format("connect %s:%d %s(%d)", host.data(), port, ec.message().data(), ec.value()), sessionid, PTYPE_ERROR);
                            }
                        });
                }
                else
                {
                    response(0, owner, amx::format("resolve %s:%d %s(%d)", host.data(), port, ec.message().data(), ec.value()), sessionid, PTYPE_ERROR);
                }
            });
    }
    return 0;
...
bool socket_server::set_enable_chunked(uint32_t fd, std::string_view flag)
{
    ...
    for (const auto& c : flag)
    {
        ...
        case 'r':
        case 'R':
            v = v | amx::enable_chunked::receive;
            break;
        case 'w':
        case 'W':
            v = v | amx::enable_chunked::send;
            break;
        default:
...
std::string amx::socket_server::getaddress(uint32_t fd)
{
	if (auto iter = connections_.find(fd); iter != connections_.end())
	{
...
bool amx::socket_server::switch_type(uint32_t fd, uint8_t new_type)
{
    if (auto iter = connections_.find(fd); iter != connections_.end())
	{
        auto c = iter->second;
