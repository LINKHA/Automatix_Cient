...
namespace amx
{
    ...
    public:
        ...
        explicit base_connection(uint32_t serviceid, uint8_t type, amx::socket_server* s, Args&&... args)
        ...
        virtual bool send(buffer_ptr_t data)
        {
            ...
            if (wq_warn_size_ != 0 && queue_.size() >= wq_warn_size_)
            {
                ...
                {
                    asio::post(socket_.get_executor(), [this, self = shared_from_this()]() {
                        error(make_error_code(amx::error::send_queue_too_big));
                    });
                    return false;
                }
            }
        ...
        void timeout(time_t now)
        {
            ...
            {
                asio::post(socket_.get_executor(), [this, self = shared_from_this()]() {
                    error(make_error_code(amx::error::read_timeout));
                });
                return;
            }
            return;
        ...
        amx::log* logger() const
        {
            return log_;
        }

        void logger(amx::log* l)
        {
            log_ = l;
        }
    ...
    protected:
        ...
        virtual void error(const asio::error_code& e, const std::string& additional = "")
        {
            ...
            std::string content =
                amx::format("{\"addr\":\"%s\",\"code\":%d,\"message\":\"%s\"}", address().data(), e.value(), message.data());
