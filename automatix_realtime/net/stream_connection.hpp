...
namespace amx
{
    ...
    protected:
        ...
        {
            asio::async_read_until(socket_, amx::streambuf(response_.get_buffer(), count), delim_,
        ...
        void read(size_t count)
        {
            ...
            asio::async_read(socket_, amx::streambuf(response_.get_buffer(), count), asio::transfer_exactly(size),
        ...
        void error(const asio::error_code& e, const std::string& additional = "") override
        {
            ...
            if (e)
            {
                if (e == amx::error::read_timeout)
                {
                    response_.write_data(amx::format("TIMEOUT %s.(%d)", e.message().data(), e.value()));
                }
                else if (e == asio::error::eof)
                {
                    response_.write_data(amx::format("EOF %s.(%d)", e.message().data(), e.value()));
                }
                else
                {
                    response_.write_data(amx::format("SOCKET_ERROR %s.(%d)", e.message().data(), e.value()));
                }
            }
