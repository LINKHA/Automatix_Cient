...
namespace amx
{
    ...
    public:
        ...
        static fs::path module_path()
        {
            ...
            if(0 == len)
                throw std::runtime_error(amx::format("module_path error: %u", GetLastError()));
            ...
            if(-1 == len)
                throw std::runtime_error(amx::format("module_path error %d", errno));
#endif
        ...
        {
            std::string result;
            std::vector<std::string> searchdir = amx::split<std::string>(path, ";");
