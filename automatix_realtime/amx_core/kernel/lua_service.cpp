...
void* lua_service::lalloc(void* ud, void* ptr, size_t osize, size_t nsize)
{
    ...
    if (l->mem > l->mem_limit)
    {
        ...
        {
            l->mem = mem;
            l->logger()->logstring(true, amx::LogLevel::Error,
                amx::format("%s Memory error current %.2f M, limit %.2f M", l->name().data(), (float)(l->mem) / mb_memory,
    ...
    else if (l->mem > l->mem_report)
    {
        l->mem_report *= 2;
        l->logger()->logstring(
            true, amx::LogLevel::Warn, amx::format("%s Memory warning %.2f M", l->name().data(), (float)l->mem / mb_memory), l->id());
    }
...
lua_service::~lua_service()
{
    logger()->logstring(true, amx::LogLevel::Info, amx::format("[WORKER %u] destroy service [%s] ", worker_->id(), name().data()), id());
}

static int protect_init(lua_State* L)
{
    const amx::service_conf* conf = (const amx::service_conf*)lua_touserdata(L, 1);
...
bool lua_service::init(const amx::service_conf& conf)
{
    mem_limit = conf.memlimit;
    name_ = conf.name;

    logger()->logstring(true, amx::LogLevel::Info, amx::format("[WORKER %u] new service [%s]", worker_->id(), name().data()), id());
...
void lua_service::dispatch(message* msg)
{
    if (!ok())
        return;
    if (cb_ctx == nullptr) {
        logger()->logstring(true, amx::LogLevel::Error, "callback not init", id());
        return;
    }
    ...
    try
    {
        ...
        {
        case LUA_ERRRUN:
            error = amx::format("dispatch %s error:\n%s", name().data(), lua_tostring(L, -1));
            break;
        case LUA_ERRMEM:
            error = amx::format("dispatch %s memory error", name().data());
            break;
        case LUA_ERRERR:
            error = amx::format("dispatch %s error in error", name().data());
            break;
        };
        ...
        if (msg->sessionid() >= 0)
        {
            logger()->logstring(true, amx::LogLevel::Error, error, id());
        }
        else
        {
