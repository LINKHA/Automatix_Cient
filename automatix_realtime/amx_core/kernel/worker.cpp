...
namespace amx
{
	...
	void worker::run()
	{
		socket_server_ = std::make_unique<amx::socket_server>(server_, this, io_ctx_);
	...
	void worker::remove_service(uint32_t serviceid, uint32_t sender, uint32_t sessionid)
	{
		asio::post(io_ctx_, [this, serviceid, sender, sessionid]() {
			if (auto s = find_service(serviceid); nullptr != s)
			{
				...
				{
					auto content = amx::format("_service_exit,name:%s serviceid:%08X", name.data(), id);
			...
			{
				server_->response(sender, amx::format("service [%08X] not found", serviceid), sessionid, PTYPE_ERROR);
			}
			});
	}
	...
	void worker::handle_one(service*& s, message&& msg)
	{
		...
		if (nullptr == s || s->id() != receiver)
		{
			...
			if (nullptr == s || !s->ok())
			{
				...
				{
					std::string hexdata = amx::hex_string({ msg.data(),msg.size() });
					std::string str = amx::format("[%08X] attempt send to dead service [%08X]: %s."
						, sender
						, receiver
						, hexdata.data());
		...
		double start_time = amx::time::clock();
		handle_message(s, std::move(msg));
		double cost_time = amx::time::clock() - start_time;
