...
namespace amx
{
    ...
    public:
        ...
        static size_t milltimestamp(std::time_t t, char* buf, size_t len)
        {
            ...
            std::tm m;
            amx::time::localtime(&now, &m);
            uint64_t ymd = (m.tm_year + 1900ULL) * amx::pow10(15)
                + (m.tm_mon + 1ULL) * amx::pow10(12)
                + m.tm_mday * amx::pow10(9)
                + m.tm_hour * amx::pow10(6)
                + m.tm_min * amx::pow10(3)
                + m.tm_sec;
