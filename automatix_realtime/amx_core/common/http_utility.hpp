...
namespace amx
{
    namespace http
    {
        using  case_insensitive_multimap = std::unordered_multimap<std::string, std::string, amx::ihash_string_view_functor_t, amx::iequal_string_view_functor_t>;
        using  case_insensitive_multimap_view = std::unordered_multimap<std::string_view, std::string_view, amx::ihash_string_view_functor_t, amx::iequal_string_view_functor_t>;
            ...
            static std::string decode(std::string_view value) noexcept {
                ...
                for (std::size_t i = 0; i < value.size(); ++i) {
                    ...
                    if (chr == '%' && i + 2 < value.size()) {
                        ...
                        auto decoded_chr = amx::string_convert<uint8_t>(hex, ec, 16);
